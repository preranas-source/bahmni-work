    ...
    .controller('TopDownAddressFieldsDirectiveController', ['$scope', 'addressHierarchyService', '$translate', function ($scope, addressHierarchyService, $translate) {
        ...
        var init = function () {
            ...
            _.each($scope.addressLevels, function (addressLevel) {
                // Allow free-text for pincode/postalCode - user can type without selecting from autocomplete
                var allowFreeText = addressLevel.addressField === 'postalCode' || addressLevel.addressField === 'pincode';
                addressLevel.isStrictEntry = !allowFreeText && ($scope.strictAutocompleteFromLevel == addressLevel.addressField || isStrictEntry);
