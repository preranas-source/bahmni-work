...
public abstract class Accessioner {
	...
	{
		analysisDone.add(StatusOfSampleUtil.getStatusID(StatusOfSampleUtil.AnalysisStatus.Finalized));
		analysisDone.add(StatusOfSampleUtil.getStatusID(StatusOfSampleUtil.AnalysisStatus.NonConforming_depricated));
		analysisDone.add(StatusOfSampleUtil.getStatusID(StatusOfSampleUtil.AnalysisStatus.Canceled));
	}
...
//		OBSERVATION_HISTORY_YES_ID = new DictionaryDAOImpl().getDictionaryByDictEntry("Demographic Response Yes (in Yes or No)").getId();
	...
	public StatusSet findStatusSet() {
		if (statusSet == null) {
			...
			if (GenericValidator.isBlankOrNull(sampleId)) {
				statusSet = StatusOfSampleUtil.getStatusSetForAccessionNumber(accessionNumber);
			} else {
				statusSet = StatusOfSampleUtil.getStatusSetForSampleId(sampleId);
			}
		}
		return statusSet;
	}
	...
	protected void populateSampleItems(List<TypeOfSampleTests> typeofSampleTestList, String collectionDate) throws Exception {
		...
		for (TypeOfSampleTests typeofSampleTest : typeofSampleTestList) {
			...
			SampleItem item = buildSampleItem(sample, toSample);
			sampleItemsAnalysis.add(new SampleItemAnalysisCollection(item, tests, collectionDate));
		}
	}

	protected SampleItem buildSampleItem(Sample sample, TypeOfSample typeofsample) {
	...
	public void completeSample() throws Exception {
		if (isAllAnalysisDone() && !StatusOfSampleUtil.getStatusID(OrderStatus.NonConforming_depricated).equals(sample.getStatus())) {
	 ...
	 * Pahill maybe we could move this to StatusOfSampleUtil?
	 */
...
*/
	...
	protected void persistRecordStatus() {
		...
		StatusOfSampleUtil.persistRecordStatusForSample(sample, newSampleStatus, patientInDB, newPatientStatus, sysUserId);
	}
